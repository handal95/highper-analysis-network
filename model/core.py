...
class LSTMGenerator(nn.Module):
    """
    An LSTM based generator.
    It expects a sequence of noise vectors as input.
    Args:
    ...
    def __init__(self, in_dim, out_dim, device=None):
        ...
        self.linear = nn.Sequential(
            nn.Linear(in_features=128, out_features=out_dim), nn.Tanh()
        )
    ...
    def forward(self, input):
        ...
        outputs = self.linear(
            recurrent_features.contiguous().view(batch_size * seq_len, 128)
        )
...
class LSTMDiscriminator(nn.Module):
    ...
    Args:
        ...
        n_layers: number of lstm layers
        hidden_dim: dimensionality of the hidden layer of lstms
    Inputs: sequence of shape (batch_size, seq_len, in_dim)git fetch
    ...
    def __init__(self, in_dim, device=None):
        ...
        self.lstm = nn.LSTM(
            input_size=in_dim, hidden_size=100, num_layers=1, batch_first=True
        )
    ...
    def forward(self, input):
        ...
        outputs = self.linear(
            recurrent_features.contiguous().view(batch_size * seq_len, 100)
        )
...
if __name__ == "__main__":
    ...
    # print("Gen is created")

    # dis = LSTMDiscriminator(seq_dim)
    # # print("Dis is created")

    noise = torch.randn(8, 16, noise_dim)
    print("Noise: ", noise.size())

    gen_out = gen(noise)
    gen_out = numpy.array(gen_out)
    print(type(gen_out))
