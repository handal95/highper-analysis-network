...
def convert_scale_domain(config, info, dataset, metaset):
    ...
    if abs(info["stat"]["skew"]) > 0:
        ...
        if "square" in info["descript"] and (
            ...
            print(
                f"[Recommand] 'Sqrt' or 'Log Sqrt' scaling is recommended for squared values"
            )
        ...
        answer = (
            ...
            if config["options"]["FIX_COLUMN_AUTO"] is False
            else auto_answer
        )
...
def convert_score_domain(info, dataset, metaset):
    ...
    if match_index is not None:
        ...
        # if answer is True:
        if True:
            dataset["train"][col] = dataset["train"][col].replace(
                scoretype[match_index]
            )
